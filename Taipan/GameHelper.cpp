...
Vector2 GameHelper::HandleAlignment(Vector2 pos, Vector2 size, TextAlignment ta) {
	Vector2 AlignedPos;
	...
	case TextHAlignment::Centre:
		AlignedPos.x = pos.x - size.x / 2; //size vector2 holds width in the x value and height in the y value
		break;
	case TextHAlignment::Right:
		AlignedPos.x = pos.x - size.x;
		break;
	default:
		AlignedPos.x = pos.x;
		break;
	...
	case TextVAlignment::Centre:
		AlignedPos.y = pos.y - size.y / 2;
		break;
	case TextVAlignment::Bottom:
		AlignedPos.y = pos.y - size.y;
		break;
	default:
		AlignedPos.y = pos.y;
		break;
...
void GameHelper::RenderText(TTF_Font* Font, string Text, Vector2 pos, TextAlignment ta, SDL_Color Colour) {
	TextSize ts{ GetTextSize(Font, Text) };
	int w{ ts.width }, h{ ts.height };
	
	GameWindow->RenderText(Font, Text.c_str(), HandleAlignment({ pos.x, pos.y }, { w, h }, ta), Colour);
}

void GameHelper::RenderTextInverse(TTF_Font* Font, string Text, int x, int y, TextAlignment ta, SDL_Color Colour, SDL_Color BackColour) {
	TextSize ts{ GetTextSize(Font,Text) };
	int w{ ts.width }, h{ ts.height };

	GameWindow->RenderTextInverse(Font, Text.c_str(), HandleAlignment({ x, y }, { w, h }, ta), Colour, BackColour);
}
...
void GameHelper::RenderTextFlash(TTF_Font* Font, string Text, int x, int y, TextAlignment ta, int period, SDL_Color Colour, SDL_Color InverseColor) {
	...
	if (Flag) {
		...
		RenderTextInverse(Font, Text, x, y, ta, InverseColor, Colour);	// Render InverseColour on Colour
	}
}
