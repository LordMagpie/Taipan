...
void Game::Update() {
	...
	gameHelper.RenderText(FontT1, "In Use:", pos, ta, dosColors[YELLOW]);
	...
	if (HoldFree < 0) {
		...
		gameHelper.RenderTextInverse(FontT1, Label, tx, y + h * 2 - 30, ta, dosColors[BLUE], dosColors[LIGHT_RED]);
	}
	else {
	...
	gameHelper.RenderTextInverse(FontT1, "$" + gameHelper.InsertCommas(Debt), tx, y + h * 8, ta, dosColors[BLACK],dosColors[LIGHT_CYAN]);
